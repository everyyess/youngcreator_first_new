...
function OutcomeComposition({ result, isElb }: { result: SimulationResult; isElb: boolean }) {
  const rows = [
    { label: '조기상환', estimate: result.outcomeStats.earlyRedemption, color: '#0A78F5' },
    { label: isElb ? '만기상환' : '만기 정상상환', estimate: result.outcomeStats.maturityProfit, color: '#00B5CD' },
    ...(!isElb ? [{ label: '원금손실', estimate: result.outcomeStats.principalLoss, color: '#F25536' }] : []),
  ]
...
function RealizedReturnChart({ distribution, samples }: { distribution: readonly DistributionPoint[]; samples: number }) {
  ...
  return <Chart title="상환 시 실현 총수익률 분포" subtitle={subtitle}>
    ...
    <ResponsiveContainer><BarChart data={visibleDistribution} margin={{ top: 24, right: 12, left: 8, bottom: 5 }}><CartesianGrid vertical={false} stroke="#CBD1E8" strokeOpacity={.65} /><XAxis type="number" dataKey="value" domain={domain} allowDataOverflow tickFormatter={pct} minTickGap={34} /><YAxis domain={[0, 'auto']} tickFormatter={pct} width={56} /><Tooltip cursor={false} content={<ReturnTooltip samples={samples} />} /><Bar dataKey="probability" fill="#668AE0" fillOpacity={.92} shape={pmfBarShape(fullRange ? 7 : 18)} activeBar={false} isAnimationActive={false} /></BarChart></ResponsiveContainer>
  </Chart>
}
