...
function DonutChart({ assets }: { assets: PortfolioAsset[] }) {
  ...
  for (const a of assets) {
    ...
    byClass[a.asset_class] = (byClass[a.asset_class] ?? 0) + pct;
  }
...
export default function ExistingPortfolioTab({
  ...
  const runAnalysis = useCallback(
    async (assets: PortfolioAsset[]) => {
      ...
      const _totalCheck = assets.reduce((s, a) => {
      ...
      try {
        ...
        // ── Step 1: 총 자산 가치 계산 (개별 자산 비중 산출 기준)
        ...
        const assetsWithWeights: PortfolioAsset[] = assets.map((a) => {
          ...
          let gain = a.gain ?? 0;
          if (!gain && a.buy_price != null && a.amount > 0) {
            if (a.amount_type === "quantity" && a.current_price != null) {
              gain = (a.current_price - a.buy_price) * a.amount;
            } else if (a.amount_type === "value" && value > 0) {
              gain = value - a.buy_price;
            }
          }
        ...
        setStressResult(runStressTest(quantInput, totalValue));
  ...
  return (
    <div className="space-y-5">
      {/* ── Section 1: Existing form (identical to original) ── */}
        ...
        <section className="space-y-5">
          <Panel icon={<WalletCards size={18} />} eyebrow="기존 포트폴리오 분석" title="보유 자산 정보">
            <TextAreaField
              ...
              onChange={(v) => setRrttllu("holdingOrDisposalPlan", v)}
            />
            <div className="grid gap-3 md:grid-cols-2">
              <TextField
                label="총 자산"
                value={formData.financial.totalAssets}
                placeholder="예. 20억 원"
                onChange={(v) => setFinancial("totalAssets", v)}
              />
              <TextField
                label="금융자산"
                value={formData.financial.financialAssets}
                placeholder="예. 8억 원"
                onChange={(v) => setFinancial("financialAssets", v)}
              />
            </div>
            <TextAreaField
              label="피하고 싶은 자산"
              ...
              onChange={(v) => setRrttllu("avoidedAssets", v)}
            />
          </Panel>
          ...
          <ResultCard icon={<ShieldCheck size={18} />} title="고객 성향 연동 요약" accent="gold">
            <ResultGrid
              rows={[
                ...
                ["한계세율(추정)", `${(tMarginal * 100).toFixed(0)}%`],
              ]}
            />
          </ResultCard>
          <ResultCard
            ...
            title="분석 전 확인 사항"
            accent={warnings.length ? "orange" : "green"}
          >
            <p className="text-sm font-semibold leading-6 text-slate-700">
              {warnings.length
                ? "고객 성향 분석 탭의 누락 정보가 기존 포트폴리오 진단에도 반영됩니다."
                : "현재 입력된 고객 정보가 기존 포트폴리오 분석에 충분히 반영되어 있습니다."}
            </p>
          </ResultCard>
        </aside>
      </div>
          ...
          <div className="overflow-x-auto rounded-lg border border-slate-200">
            <table className="w-full text-sm">
              <thead className="border-b border-slate-200 bg-slate-50">
                <tr>
                  {["종목명", "자산군", "입력방식", "금액(원) / 수량·현재가", "매수단가", "헤지", "리뷰", ""].map((h) => (
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              updateRow(i, { current_price: cp, current_value: a.amount * cp });
                            }}
                          />
                        </div>
                      ) : (
