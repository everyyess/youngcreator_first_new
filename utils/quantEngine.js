 ...
 */
export function getAssetProxy(name = '', assetClass = '') {
...
/**
 * Yahoo Finance에서 3개년 월봉 시계열 수집 (해외주식/ETF/금/원자재)
 * @param {string} ticker  Yahoo Finance 티커 (예: 'AAPL', 'GLD', 'TLT')
 * @returns {Promise<{ticker:string, dates:string[], closes:number[], returns:number[]}|null>}
 */
export async function fetchYahooFinanceHistory(ticker) {
  const endTs   = Math.floor(Date.now() / 1000);
  const startTs = endTs - 3 * 365 * 24 * 3600; // 3년 전 Unix timestamp
  ...
  try {
    ...
    console.warn(`[quantEngine] Yahoo Finance 연동 실패 (${ticker}):`, err?.message);
    return null;
  }
}
 ...
 */
export async function fetchAssetReturns(name, assetClass) {
  ...
  // 해외주식·해외채권·금·리츠 → Yahoo Finance 실 데이터
  const ticker = name.toUpperCase().replace(/\s+/g, '');
  const data   = await fetchYahooFinanceHistory(ticker);
  if (data && data.returns.length >= 6) return data.returns;

  // 국내주식 또는 fallback → 프록시 기반 현실적 시계열 생성
  const proxy = getAssetProxy(name, assetClass);
  return buildProxyMonthlyReturns(proxy.annVol, 0.06, 35);
}
...
export function diversificationScore(assets) {
  ...
  const heatmap = Array.from({ length: n }, (_, i) =>
    Array.from({ length: n }, (_, j) =>
      i === j ? 1 : +correlation(assets[i].returns, assets[j].returns).toFixed(4)
    )
  );
...
export async function runQuantAnalysis(rawAssets, t_marginal, marketReturns) {
  ...
  // ── Step 2: 각 자산 월별 수익률 시계열 병렬 수집
  const withReturns = await Promise.all(
    tagged.map(async asset => ({
      ...asset,
      returns: await fetchAssetReturns(asset.name, asset.assetClass),
    }))
  );
  ...
  // ── Step 4: 시장 수익률 (미제공 시 Mock 사용)
  const mktRet = marketReturns ?? await _mockMarketReturns(portRet.length);

  // ── Step 5: 총자산 · 프록시 가중 리스크 팩터 · 연환산 지표
  const totalPortValue = rawAssets.reduce((s, a) => s + (a.value ?? 0), 0);

  // 자산별 프록시 상수를 비중 가중 합산 → 포트폴리오 수준 리스크 팩터
  const proxyMetrics = tagged.reduce((acc, asset, i) => {
    const proxy = getAssetProxy(asset.name, asset.assetClass);
    const w     = rawAssets[i]?.weight ?? 0;
    return {
      annVol: acc.annVol + w * proxy.annVol,
      mdd:    acc.mdd    + w * proxy.mdd,
      beta:   acc.beta   + w * proxy.beta,
    };
  }, { annVol: 0, mdd: 0, beta: 0 });

  const annualRet = mean(portRet) * 12;
  const beta      = proxyMetrics.beta;   // 프록시 가중 베타 적용
  ...
  const performance = {
    ...
    jensensAlpha:           jensensAlpha(annualRet, beta),
  };
...
function resolveAssetMeta(asset) {
  ...
  return {
    ...
    is_hedged: asset._meta?.is_hedged  ?? false,
...
function getScenario1Shock(asset) {
  const { assetClass, theme, country, is_hedged, name } = resolveAssetMeta(asset);
...
function getScenario2Shock(asset) {
  const { assetClass, theme, country } = resolveAssetMeta(asset);
  const isForeign    = country !== '한국' ||
    [ASSET_CLASS.FOREIGN_STOCK, ASSET_CLASS.FOREIGN_BOND, ASSET_CLASS.GOLD].includes(assetClass);
...
function getScenario3Shock(asset) {
  const { assetClass, theme, country, is_hedged } = resolveAssetMeta(asset);
  const isForeign    = country !== '한국' ||
  ...
  if (isForeign  && !is_hedged)    return +0.12;   // 환차익
  if (isForeign  &&  is_hedged)    return -0.02;   // 헤지 비용
