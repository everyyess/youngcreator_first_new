...
각 종목에서 추출할 정보:
- name: 종목명 — 표에 인쇄된 글자를 한 글자도 바꾸지 말고 그대로 옮길 것 (예: "삼성전자"). 종목명 옆의 "*" 표시만 name에서 제외하고 isNonCoverage로 별도 표시
...
중요 규칙:
- 종목명은 절대 축약·의역·교정하지 말 것. 익숙한 회사명으로 바꾸거나 글자를 빼먹으면 안 된다
  (예: "한중엔시에스"를 "한중엔시스"로 줄이면 안 됨, "LIG디펜스앤에어로스페이스"처럼 두 줄로 줄바꿈된 이름은 한 줄로 이어붙일 것)
- 종목명에 한글·영문·숫자가 섞여 있으면 표기 그대로 유지하고, 모르는 회사여도 보이는 대로 적을 것
