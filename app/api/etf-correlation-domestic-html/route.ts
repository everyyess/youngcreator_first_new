/**
 * /api/etf-correlation-domestic-html  GET ?strategy=balanced&k=3
...
function greedyOptimal(
  ...
  // maxVol 기준은 극단 outlier 하나가 기준이 되면 일반 ETF까지
...
function monteCarloWeights(
  ...
  const hasMultipleClasses = new Set(assetClasses).size > 1;
...
function computePeriodData(
  ...
  for (const [pname, nDays] of Object.entries(PERIOD_SLICES)) {
    ...
    const sliceVols: Record<string, number> = {};
    const sliceAnnVols: Record<string, number> = {};
    const sliceAnnRets: Record<string, number> = {};
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    return new Response(
      '<h1>ETF 데이터 로드 실패</h1><p>data/etf_domestic_b1.json, etf_domestic_b2.json, template_domestic.html 파일을 확인하세요.</p>',
