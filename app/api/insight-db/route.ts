...
export type InsightSource = "youtube" | "blog" | "telegram" | "news" | "report";
...
const day=(value:unknown,fallback:unknown)=>text(value).slice(0,10)||text(fallback).slice(0,10);

export async function GET(req: NextRequest) {
  const db=getInsightSupabase(req);
  if(!db) return NextResponse.json(insightDbUnavailable(),{status:401});
  const queries = await Promise.all([
    db.from("youtube_summaries").select("id,title,summary,notes,topics,companies,macro,date,created_at,video_url,channel_name").order("created_at",{ascending:false}).limit(1000),
    db.from("blog_summaries").select("id,title,summary,notes,topics,companies,macro,date,created_at,post_url,feed_name").order("created_at",{ascending:false}).limit(1000),
  ...
  const sources:InsightSource[]=["youtube","blog","telegram","news","report"];
  const skippedSources:InsightSource[]=[];
  const items:InsightItem[]=[];
  queries.forEach((result,index)=>{
    const source=sources[index];
    if(result.error){skippedSources.push(source);return;}
    for(const raw of result.data??[]){const row=raw as Row;items.push({
      ...
      notes:text(row.notes),topics:list(row.topics??row.topic_tags).filter((tag)=>tag!=="News"),companies:list(row.companies??row.company_tags),macro:list(row.macro??row.macro_tags),
      date:day(row.date??row.msg_date??row.published_date,row.created_at),createdAt:text(row.created_at),url:text(row.video_url??row.post_url??row.link??row.url)||null,
      meta:text(row.channel_name??row.feed_name??row.channel??row.category??row.broker),
    });}
  });
  items.sort((a,b)=>b.date.localeCompare(a.date)||b.createdAt.localeCompare(a.createdAt));
  return NextResponse.json({items,skippedSources,counts:Object.fromEntries(sources.map((source)=>[source,items.filter((item)=>item.source===source).length]))});
}
...
export async function DELETE(req: NextRequest) {
  ...
  const table=body.source&&({youtube:"youtube_summaries",blog:"blog_summaries",telegram:"telegram_saved",news:"news_articles",report:"report_db"} as const)[body.source];
