      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s.label} className="flex items-start">
            <div className="flex w-20 flex-col items-center gap-1.5">
              <div
                className={`flex h-8 w-8 items-center justify-center rounded-full border-2 text-xs font-bold transition-colors ${
                  isDone
                    ? "border-[#003CDC] bg-[#003CDC] text-white"
                    : isActive
                    ? "border-[#003CDC] bg-white text-[#003CDC]"
              ...
              <span
                className={`text-[11px] font-semibold whitespace-nowrap ${
                  isDone || isActive ? "text-[#003CDC]" : "text-slate-300"
                }`}
              >
                {s.label}
              </span>
            ...
            {i < steps.length - 1 && (
              <div className={`mt-4 h-0.5 w-8 ${isDone ? "bg-[#003CDC]" : "bg-slate-200"}`} />
            )}
          </div>
        );
      })}
            ...
            {stage === "error" ? (
              <>
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    onClick={runFlow}
                    className="rounded-md bg-[#003CDC] px-4 py-2 text-[13px] font-bold text-white hover:bg-[#0A2FA8]"
                  >
                    다시 시도
                  </button>
                </div>
              ...
              <>
                ...
                <>
                <div className="mb-6 text-center text-[15px] font-bold text-[#003CDC]">AI 멀티에이전트 검증 진행 중</div>
                <StepperRow />
