            ...
            return (
              <div
                ...
                className={`rounded-lg border p-4 transition ${
                  state.checked ? "border-[#003CDC]/30 bg-[#EEF2FE]" : "border-slate-200 bg-white"
                }`}
              >
                  ...
                  <label className="flex cursor-pointer items-center gap-2 text-[12px] font-bold text-slate-600">
                    <span
                      ...
                      className={`flex h-5 w-5 items-center justify-center rounded border-2 transition ${
                        state.checked ? "border-[#003CDC] bg-[#003CDC]" : "border-slate-300 bg-white"
                      }`}
                    >
                      {state.checked && <CheckCircle2 size={14} className="text-white" />}
                    </span>
                    <span className={state.checked ? "text-[#003CDC]" : ""}>검토 완료</span>
                  </label>
                </div>
                ...
                {isEditing ? (
                  <textarea
                    ...
                    autoFocus
                    rows={5}
                    className="w-full rounded-md border border-slate-300 px-3 py-2 text-[13px] leading-relaxed text-slate-700 outline-none focus:border-[#003CDC]"
                  />
                ) : (
                  <p
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-md border border-slate-200 px-2.5 py-1.5 text-[12px] text-slate-600 outline-none focus:border-[#003CDC]"
                />
              </div>
            );
          })}
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 rounded-md px-4 py-2 text-[13px] font-bold text-white transition ${
                allChecked ? "bg-[#003CDC] hover:bg-[#0A2FA8]" : "cursor-not-allowed bg-slate-300"
              }`}
            >
