        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          <label className="flex items-center gap-1.5 text-[11px] text-slate-500">
            <input
              ...
              className="h-3.5 w-3.5 accent-[#003CDC]"
            />
            30초 자동 갱신
          </label>
        </div>
            ...
            return (
              <div
                ...
                className={`flex items-center gap-2 rounded-lg border px-3 py-2.5 transition ${
                  isActive ? "border-[#003CDC] bg-[#003CDC]/5" : "border-slate-200 bg-white"
                }`}
              >
                <input
                  type="checkbox"
                  ...
                  className="h-4 w-4 accent-[#003CDC]"
                />
                ...
                {isActive && activeConditions.length > 1 && (
                  <label className="flex items-center gap-1 text-[11px] font-semibold text-slate-500">
                    <input
                      ...
                      className="h-3 w-3 accent-[#003CDC]"
                    />
                    기준
                  </label>
                )}
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="rounded-full bg-[#003CDC] px-2.5 py-1 text-[11px] font-semibold text-white">
                기준: {baseDef.label}
              </span>
                    ...
                    {activeConditions
                      ...
                      .map((c) => (
                        <th key={c.id} className="px-3 py-2 text-left font-medium">
                          {c.columnLabel}
                          {c.id === baseCondition && <span className="ml-1 text-[10px] text-[#003CDC]">●기준</span>}
                        </th>
                      ))}
                  </tr>
                </thead>
