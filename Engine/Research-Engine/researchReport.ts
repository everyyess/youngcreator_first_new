...
9. PB가 즉시 확인해야 할 핵심 결론, 중요 수치, 주요 리스크와 포트폴리오 대응 문구를 직접 판단해 **굵게** 표시합니다. 특히 중요한 [인용]·[판단] 문장에는 핵심 구절을 반드시 **굵게** 표시해 출력물에서 굵은 밑줄 강조가 함께 적용되게 합니다. 강조는 문단당 최대 1개, 보고서 전체 8~15개로 제한하며 문장 전체를 굵게 만들지 않습니다.
...
수집된 자료는 "${result.keyword}" 자체의 절대 수준보다 **정책 기대, 채권시장 가격, 경기와 물가 신호가 어떻게 함께 움직이는지**를 확인해야 한다는 점을 보여줍니다. [판단(1)]

**2. 전개 시계열**
${timeline || "날짜가 확인되는 자료가 없어 국면 전개를 확정하기 어렵습니다. [판단(2)]"}

시간 순서상 같은 방향의 제목이 반복되는지, 반대 방향의 새로운 재료가 등장하는지를 구분해야 **추세 지속과 단기 노이즈**를 나눌 수 있습니다. [판단(2)]

**3. 자산군·섹터별 파급 경로**
금리 기대가 높아지는 경우 **채권 가격에는 하방 압력, 성장주에는 밸류에이션 부담**이 커질 수 있습니다. 반대로 금리 기대가 낮아지면 해당 경로가 완화될 수 있습니다. [판단(3)]

환율과 금융주의 반응은 금리의 방향만으로 결정되지 않고, 변화 원인이 경기 개선인지 물가 압력인지에 따라 달라질 수 있으므로 **원인과 시장 반응을 함께 확인**해야 합니다. [판단(4)]

**4. 우호적·비우호적 시나리오**
우호적 시나리오는 **물가 압력이 완화되면서 경기 훼손 없이 금리 부담이 낮아지는 경우**입니다. 이때 채권과 성장주의 할인율 부담이 완화되는지 확인해야 합니다. [판단(5)]

비우호적 시나리오는 **금리 기대가 재상승하거나 경기 둔화가 예상보다 빠르게 나타나는 경우**입니다. 전자는 밸류에이션 부담을, 후자는 실적 전망 하향 위험을 키울 수 있습니다. [판단(6)]
...
**5. PB 체크포인트**
1. **중앙은행의 정책 신호와 시장의 금리 경로 기대**가 같은 방향인지 확인합니다. [판단(7)]
2. **장단기 금리와 환율의 동행 여부**를 확인해 충격의 성격을 구분합니다. [판단(8)]
3. 물가·고용·성장 지표 발표 뒤 **채권과 주식의 반응**이 기존 국면을 강화하는지 확인합니다. [판단(9)]
4. 고객 포트폴리오의 **듀레이션, 성장주 비중, 환 노출**을 시나리오별로 점검합니다. [판단(10)]
